    ...
    load() {
        ...
        xhr.onreadystatechange = function () {
            ...
            for (let i = 0; i < numPoints; i++) {
                let x =
                    sourceView.getFloat32(i * 17 + 0, true) +
                    node.boundingBox.min.x
                let y =
                    sourceView.getFloat32(i * 17 + 4, true) +
                    node.boundingBox.min.y
                let z =
                    sourceView.getFloat32(i * 17 + 8, true) +
                ...
                if (xhr.readyState === 4 && xhr.status === 200) {
                    ...
                    geometry.boundingBox = new THREE.Box3(
                        new THREE.Vector3().fromArray(
                            response.BoundingBox.slice(0, 3),
                        ),
                        new THREE.Vector3().fromArray(
                            response.BoundingBox.slice(3, 6),
                        ),
                    )
                    if (response.Spacing) {
                    ...
                    let offset = geometry.boundingBox.min
                        .clone()
                        .multiplyScalar(-1)
        ...
        xhr.onreadystatechange = () => {
            ...
            for (let i = 0; i < numNodes; i++) {
                ...
                if (stack.length > 0) {
                    ...
                    let radius =
                        root.boundingBox.getSize(new THREE.Vector3()).length() /
                        2
