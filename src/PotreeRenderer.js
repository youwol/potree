    ...
    updateBuffer(geometry) {
        ...
        for (let attributeName in geometry.attributes) {
            ...
            if (!webglBuffer.vbos.has(attributeName)) {
                ...
                webglBuffer.vbos.get(attributeName).version =
        ...
        for (let node of nodes) {
...

            {
                ...
                if (material.clipPolygons && material.clipPolygons.length > 0) {
                    ...
                    const lClipPolygonVP =
                        shader.uniformLocations['uClipPolygonWVP[0]']
                    gl.uniformMatrix4fv(
                        lClipPolygonVP,
                        false,
                        flattenedMatrices,
            ...
            shader.setUniform1f(
                'uNodeSpacing',
                node.geometryNode.estimatedSpacing,
            )
            ...
            if (shadowMaps.length > 0) {
...

                {
                    ...
                    const lWorldView =
                        shader.uniformLocations['uShadowWorldView[0]']
...

                {
                    let flattenedMatrices = [].concat(
                        ...shadowMaps.map(
                            (sm) => sm.camera.projectionMatrix.elements,
                        ),
                    )
            ...
            if (geometry.attributes['gps-time']) {
                ...
                let offset =
                    -(globalRange[0] - initialRange[0]) / initialRangeSize
                ...
                let normalizedClipRange = [
                    (uFilterGPSTimeClipRange[0] - globalRange[0]) /
                        globalRangeSize,
                    (uFilterGPSTimeClipRange[1] - globalRange[0]) /
                        globalRangeSize,
                ]
            ...
            let isExtraAttribute =
                attributeLocations[material.activeAttributeName] ===
                    undefined &&
                Object.keys(geometry.attributes).includes(
            ...
            if (isExtraAttribute) {
                ...
                {
                    const attExtra =
                        octree.pcoGeometry.pointAttributes.attributes.find(
                            (a) => a.name === attName,
                        )
    ...
    renderOctree(octree, nodes, camera, target, params = {}) {
        ...
        if (material.pointSizeType >= 0) {
            if (
                ...
                let vnNodes =
                    params.vnTextureNodes != null
                        ? params.vnTextureNodes
                        : nodes
...

        {
            ...
            //if(material.needsUpdate){
            {
                ...
                let numSnapshots = material.snapEnabled
                    ? material.numSnapshots
                    : 0
                let numClipBoxes =
...

        {
            ...
            if (
                material.clipBoxes.length + material.clipPolygons.length ===
                0
            ) {
            ...
            gl.activeTexture(gl.TEXTURE0 + currentTextureBindingPoint)
            gl.bindTexture(
                classificationTexture.target,
                classificationTexture.id,
            )
            currentTextureBindingPoint++
            ...
            if (material.snapEnabled === true) {
                {
                    ...
                    const lSnapshotDepth =
                        shader.uniformLocations['uSnapshotDepth[0]']
                    ...
                    for (let i = 0; i < 5; i++) {
                        ...
                        let textureDepth =
                            material.uniforms[`uSnapshotDepth`].value[i]
                        ...
                        let snapTexture =
                            this.threeRenderer.properties.get(
                                texture,
                            ).__webglTexture
                ...
                {
                    let flattenedMatrices = [].concat(
                        ...material.uniforms.uSnapView.value.map(
                            (c) => c.elements,
                        ),
                    )
                ...
                {
                    let flattenedMatrices = [].concat(
                        ...material.uniforms.uSnapProj.value.map(
                            (c) => c.elements,
                        ),
                    )
                ...
                {
                    let flattenedMatrices = [].concat(
                        ...material.uniforms.uSnapProjInv.value.map(
                            (c) => c.elements,
                        ),
                    )
                    const lSnapProjInv =
                        shader.uniformLocations['uSnapProjInv[0]']
                    gl.uniformMatrix4fv(lSnapProjInv, false, flattenedMatrices)
                }
                {
                    let flattenedMatrices = [].concat(
                        ...material.uniforms.uSnapViewInv.value.map(
                            (c) => c.elements,
                        ),
                    )
                    const lSnapViewInv =
                        shader.uniformLocations['uSnapViewInv[0]']
