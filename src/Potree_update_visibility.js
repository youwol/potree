...
export function updateVisibilityStructures(pointclouds, camera, renderer) {
    ...
    for (let i = 0; i < pointclouds.length; i++) {
        ...
        let camMatrixObject = new THREE.Matrix4()
            .multiply(worldI)
            .multiply(view)
...
export function updateVisibility(pointclouds, camera, renderer) {
    ...
    let numVisiblePointsInPointclouds = new Map(
    ...
    while (priorityQueue.size() > 0) {
        ...
        if (true && clipBoxes.length > 0) {
            ...
            for (let clipBox of clipBoxes) {
                ...
                let toPCObject = pcWorldInverse.multiply(
                    clipBox.box.matrixWorld,
                )

                let px = new THREE.Vector3(+0.5, 0, 0).applyMatrix4(
                    pcWorldInverse,
                )
                let nx = new THREE.Vector3(-0.5, 0, 0).applyMatrix4(
                    pcWorldInverse,
                )
                let py = new THREE.Vector3(0, +0.5, 0).applyMatrix4(
                    pcWorldInverse,
                )
                let ny = new THREE.Vector3(0, -0.5, 0).applyMatrix4(
                    pcWorldInverse,
                )
                let pz = new THREE.Vector3(0, 0, +0.5).applyMatrix4(
                    pcWorldInverse,
                )
                let nz = new THREE.Vector3(0, 0, -0.5).applyMatrix4(
                    pcWorldInverse,
                )
