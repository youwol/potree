...
export class EarthControls extends EventDispatcher {
    constructor(viewer) {
        ...
        let drag = (e) => {
            ...
            if (e.drag.mouse === MOUSE.LEFT) {
                ...
                pivotToCamTarget.applyAxisAngle(
                    new THREE.Vector3(0, 0, 1),
                    yawDelta,
                )

                let newCam = new THREE.Vector3().addVectors(
                    this.pivot,
                    pivotToCam,
                )
    ...
    zoomToLocation(mouse) {
...

        {
            ...
            tween.onUpdate(() => {
                let t = value.x
                this.scene.view.position.x =
                    (1 - t) * startPos.x + t * targetPos.x
                this.scene.view.position.y =
                    (1 - t) * startPos.y + t * targetPos.y
                this.scene.view.position.z =
                    (1 - t) * startPos.z + t * targetPos.z

                this.scene.view.radius =
    ...
    update(delta) {
        ...
        if (this.pivotIndicator.visible) {
            let distance = this.pivotIndicator.position.distanceTo(
                view.position,
            )
