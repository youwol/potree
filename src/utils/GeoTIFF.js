        ...
        static read(data) {
            ...
            while (IFDsRead || i < 100) {
                ...
                for (let i = 0; i < numEntries; i++) {
                    ...
                    let type = Type.fromValue(
                        entryBuffer.readUInt16LE(i * 12 + 2),
                    )
            ...
            for (let i = 0; i < numStrips; i++) {
                ...
                let stripData = data.slice(
                    stripOffset,
                    stripOffset + stripBytes,
                )
            ...
            for (let ifd of ifds) {
                ...
                if (ifd.count === 1 && ifd.type.bytes <= 4) {
                    ...
                    if (ifd.type === Type.ASCII) {
                        valueBuffer.set(
                            new Uint8Array(
                                ifd.value.split('').map((c) => c.charCodeAt(0)),
                            ),
                        )
                    } else {
