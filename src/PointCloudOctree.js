...
export class PointCloudOctree extends PointCloudTree {
    constructor(geometry, material) {
...

        {
            ...
            for (let attributeName of priorityQueue) {
                let attribute =
                    this.pcoGeometry.pointAttributes.attributes.find(
    ...
    updateMaterial(material, visibleNodes, camera, renderer) {
        ...
        material.uniforms.octreeSize.value =
            this.pcoGeometry.boundingBox.getSize(new THREE.Vector3()).x
    }
    ...
    intersectsPoint(position) {
        let rootAvailable =
            this.pcoGeometry.root && this.pcoGeometry.root.geometry
        ...
        if (typeof this.signedDistanceField === 'undefined') {
            ...
            for (let i = 0; i < n; i = i + 3) {
                ...
                const ix = parseInt(
                    Math.min(
                        resolution * (x / boundingBox.max.x),
                        resolution - 1,
                    ),
                )
                const iy = parseInt(
                    Math.min(
                        resolution * (y / boundingBox.max.y),
                        resolution - 1,
                    ),
                )
                const iz = parseInt(
                    Math.min(
                        resolution * (z / boundingBox.max.z),
                        resolution - 1,
                    ),
                )
    ...
    pick(viewer, camera, ray, params = {}) {
...

        {
            ...
            if (params.pickClipped) {
                ...
                pickMaterial.uniforms.clipBoxes =
                    this.material.uniforms.clipBoxes
        ...
        let y = parseInt(
            clamp(pixelPos.y - (pickWindowSize - 1) / 2, 0, height),
        )
